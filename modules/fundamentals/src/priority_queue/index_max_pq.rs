//! Indexed maximum priority queue implementation using a binary heap.
//!
//! The `IndexMaxPQ` data structure represents an indexed priority queue of generic keys.
//! It supports the usual insert and delete-the-maximum operations, along with delete and
...
//!
//! ```
//! use algs4_fundamentals::priority_queue::IndexMaxPQ;
//!
//! let mut pq = IndexMaxPQ::new(10);
//! pq.insert(0, 5);
//! pq.insert(1, 3);
//! pq.insert(2, 7);
//!
//! assert_eq!(pq.max_index(), Some(2));
...
/// An indexed maximum priority queue implemented with a binary heap.
///
...
impl<T: Ord> IndexMaxPQ<T> {
    ...
    /// use algs4_fundamentals::priority_queue::IndexMaxPQ;
    ///
    /// let pq: IndexMaxPQ<i32> = IndexMaxPQ::new(10);
    /// assert!(pq.is_empty());
    /// ```
    ...
    /// Returns a reference to a maximum key.
    ///
    /// Returns `None` if the priority queue is empty.
    ///
    /// # Examples
    ...
    /// assert_eq!(pq.del_max(), Some(0));
    /// ```
    pub fn del_max(&mut self) -> Option<usize> {
        if self.n == 0 {
            return None;
        }
        ...
        debug_assert_eq!(self.pq[self.n + 1], max);
        self.qp[max] = None;
        self.keys[max] = None;
...
mod tests {
    ...
    fn test_complex_scenario() {
        ...
        // Delete one
        pq.delete(2);
        assert_eq!(pq.max_index(), Some(4));

        // Insert another
        pq.insert(7, 200);
        assert_eq!(pq.max_index(), Some(7));
    }
}
