//! Dijkstra's shortest path algorithm for undirected edge-weighted graphs with non-negative weights.
...
/// undirected edge-weighted graph using Dijkstra's algorithm.
///
...
/// # Panics
///
/// Panics if the edge-weighted graph has edges with negative weights.
///
/// # Examples
///
/// ```
/// use algs4_graphs::{EdgeWeightedGraph, Edge, DijkstraUndirectedSP};
///
/// let mut graph = EdgeWeightedGraph::new(8);
/// graph.add_edge(Edge::new(0, 1, 5.0));
/// graph.add_edge(Edge::new(0, 4, 9.0));
/// graph.add_edge(Edge::new(0, 7, 8.0));
/// graph.add_edge(Edge::new(1, 2, 12.0));
/// graph.add_edge(Edge::new(1, 3, 15.0));
/// graph.add_edge(Edge::new(1, 7, 4.0));
/// graph.add_edge(Edge::new(2, 3, 3.0));
/// graph.add_edge(Edge::new(2, 6, 11.0));
/// graph.add_edge(Edge::new(3, 6, 9.0));
/// graph.add_edge(Edge::new(4, 5, 4.0));
/// graph.add_edge(Edge::new(4, 6, 20.0));
/// graph.add_edge(Edge::new(4, 7, 5.0));
/// graph.add_edge(Edge::new(5, 2, 1.0));
/// graph.add_edge(Edge::new(5, 6, 13.0));
/// graph.add_edge(Edge::new(7, 5, 6.0));
///
/// let sp = DijkstraUndirectedSP::new(&graph, 0);
///
/// assert!(sp.has_path_to(6));
/// assert!(sp.dist_to(6).is_some());
/// ```
#[derive(Debug, Clone)]
pub struct DijkstraUndirectedSP {
    dist_to: Vec<f64>,          // dist_to[v] = distance of shortest s->v path
    edge_to: Vec<Option<Edge>>, // edge_to[v] = last edge on shortest s->v path
}

impl DijkstraUndirectedSP {
    /// Computes a shortest-paths tree from the source vertex `s` to every other
    /// vertex in the undirected edge-weighted graph `graph`.
    ///
    /// # Arguments
    ///
    /// * `graph` - The undirected edge-weighted graph
    /// * `s` - The source vertex
    ///
    /// # Panics
    ///
    ...
    /// ```
    pub fn new(graph: &EdgeWeightedGraph, s: usize) -> Self {
        ...
        while !pq.is_empty() {
            ...
            for &e in graph.adj(v) {
                Self::relax(&mut dist_to, &mut edge_to, &mut pq, e, v);
            }
        }

        DijkstraUndirectedSP { dist_to, edge_to }
    }
    ...
    /// ```
    pub fn path_to(&self, v: usize) -> Option<Vec<Edge>> {
...
mod tests {
    ...
    #[test]
    fn test_dijkstra_undirected_basic() {
    ...
    fn test_path_to() {
        ...
        // Verify path weight matches dist_to
        let mut total_weight = 0.0;
        for e in &path {
            total_weight += e.weight();
        }
        assert_eq!(sp.dist_to(6), Some(total_weight));
    }
    ...
    fn test_self_loop() {
        ...
        graph.add_edge(Edge::new(0, 1, 1.0));
        graph.add_edge(Edge::new(1, 1, 0.5));
        graph.add_edge(Edge::new(1, 2, 2.0));
    ...
    fn test_negative_weight() {
        ...
        DijkstraUndirectedSP::new(&graph, 0);
    }
